...
function tempToRgb(kelvin: number): RgbColor {
  ...
  const blue =
    hectokelvin >= 66
      ...
      ? 0
      : pxConstrain(
          138.5177312231 * Math.log(hectokelvin - 10) - 305.0447927307
        );
...
class Range3D {
  ...
  contains(point: Coords3D): boolean {
    return (
      point[0] >= this.lowerBound[0] &&
      point[0] <= this.upperBound[0] &&
      point[1] >= this.lowerBound[1] &&
      point[1] <= this.upperBound[1] &&
      point[2] >= this.lowerBound[2] &&
      point[2] <= this.upperBound[2]
    );
  }
}
...
class Universe {
  ...
  translateY(deltaY: number): void {
    ...
    this.cleanStars();
    this.populateStars(
      new Range3D(this.size.lowerBound, [
        this.size.upperBound[0],
        this.size.lowerBound[1] + deltaY,
  ...
  resize(newSize: Range3D): void {
    if (newSize.upperBound[0] > this.size.upperBound[0]) {
      this.populateStars(
        new Range3D(
          [
            this.size.upperBound[0],
            this.size.lowerBound[1],
            this.size.lowerBound[2],
          ],
          [
            newSize.upperBound[0],
            this.size.upperBound[1],
            this.size.upperBound[2],
          ]
        )
      );
    ...
    if (newSize.lowerBound[0] < this.size.lowerBound[0]) {
      this.populateStars(
        new Range3D(
          [
            newSize.lowerBound[0],
            this.size.lowerBound[1],
            this.size.lowerBound[2],
          ],
          [
            this.size.lowerBound[0],
            this.size.upperBound[1],
            this.size.upperBound[2],
          ]
        )
    ...
    if (newSize.upperBound[2] > this.size.upperBound[2]) {
      this.populateStars(
        new Range3D(
          ...
          [newSize.upperBound[0], newSize.lowerBound[1], newSize.upperBound[2]]
        )
      );
    }
    if (newSize.lowerBound[2] < this.size.lowerBound[2]) {
      this.populateStars(
        new Range3D(
          [newSize.lowerBound[0], newSize.lowerBound[1], newSize.lowerBound[2]],
          [
            newSize.upperBound[0],
            newSize.lowerBound[1],
            this.size.lowerBound[2],
...
class Observer {
  ...
  setWindow(windowBound: Coords2D): void {
    if (
      windowBound[0] === this.#window?.upperBound[0] &&
  ...
  flatten(): Circle[] {
    ...
    for (const star of this.universe.stars) {
      ...
      const flux = star.luminosity / (star.position[2] - this.location[2]) ** 2;
      const apparentRadius = Math.sqrt(flux);

      if (
        position[0] < this.#window.lowerBound[0] - apparentRadius ||
      ...
      const [renderedRadius, opacity] =
        apparentRadius >= 1 ? [apparentRadius, 1] : [1, flux];
...
document.addEventListener("DOMContentLoaded", function () {
  const canvasElt = <HTMLCanvasElement>(
  ...
  const obs = new Observer(1, 3, 1e-4, [
    canvasElt.width / 2,
    canvasElt.height / 2,
  ]);
